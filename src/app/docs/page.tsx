...
function CodeBlock({ children }: { children: string }) {
  return (
    <pre className="text-[13px] leading-relaxed text-[#5a5248] bg-[#f5f0ea] rounded-2xl p-5 overflow-x-auto border border-[#e8e0d4]">
      <code>{children}</code>
    </pre>
  );
}
...
function Endpoint({ method, path, description, children }: {
  ...
  return (
    <div className="border border-[#e8e0d4] rounded-2xl overflow-hidden" id={path.replace(/\//g, "-").slice(1)}>
      <div className="p-5 md:p-6 bg-[#f5f0ea]">
        <div className="flex items-center gap-3 mb-2">
          <span className="text-[11px] font-mono font-semibold text-[#6b7c5e] bg-[#6b7c5e]/10 px-2.5 py-1 rounded-lg">{method}</span>
          <code className="text-[14px] md:text-[15px] font-mono text-[#3a3530]">{path}</code>
        </div>
        <p className="text-[14px] text-[#8a8078]">{description}</p>
      </div>
...
function ParamTable({ params }: { params: { name: string; type: string; required: boolean; description: string }[] }) {
  return (
    <div>
      <p className="text-[11px] uppercase tracking-[0.18em] text-[#a09080] font-medium mb-3">Parameters</p>
      <div className="space-y-2">
        {params.map((p) => (
          <div key={p.name} className="flex items-start gap-3 text-[13px]">
            <code className="text-[#6b7c5e] font-mono shrink-0">{p.name}</code>
            <span className="text-[#b0a490] shrink-0">{p.type}</span>
            {p.required && <span className="text-[10px] text-[#c09070] bg-[#c09070]/10 px-1.5 py-0.5 rounded font-medium shrink-0">required</span>}
            <span className="text-[#8a8078]">{p.description}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function DocsPage() {
  return (
    <main className="min-h-screen bg-[#faf8f4]">
      ...
      <nav className="w-full max-w-[860px] mx-auto px-6 py-6 flex items-center justify-between">
        ...
        <Link href="/app"
          className="text-[13px] font-medium text-white bg-[#6b7c5e] px-4 py-2 rounded-xl hover:bg-[#5a6b4e] transition-colors">
          Open App
        </Link>
      </nav>

      <div className="max-w-[860px] mx-auto px-6 pt-12 md:pt-20 pb-20">
        <p className="text-[11px] uppercase tracking-[0.25em] text-[#6b7c5e] font-medium mb-4">API Reference</p>
        <h1 className="font-[var(--font-serif)] text-[32px] md:text-[44px] text-[#3a3530] leading-tight mb-4">
          WeatherWear API
        </h1>
        <p className="text-[16px] text-[#8a8078] leading-relaxed max-w-[560px] mb-6">
          Get AI-powered outfit recommendations and packing lists based on real-time weather data. All endpoints return JSON.
        </p>

        <div className="rounded-xl bg-[#6b7c5e]/8 p-4 mb-14">
          <p className="text-[13px] text-[#6b7c5e] leading-relaxed">
            <span className="font-medium">Base URL:</span>{" "}
            <code className="font-mono">https://weatherwear-blush.vercel.app/api</code>
          </p>
          <p className="text-[12px] text-[#8a9a7e] mt-1">
        ...
        <div className="space-y-8">
          ...
          <Endpoint method="GET" path="/api/outfit-day" description="Get a full outfit recommendation for a specific day.">
            ...
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-[#a09080] font-medium mb-3">Example request</p>
              <CodeBlock>{`curl "https://weatherwear-blush.vercel.app/api/outfit-day?q=New+York&day=0"`}</CodeBlock>
            </div>

            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-[#a09080] font-medium mb-3">Response</p>
              <CodeBlock>{`{
  ...
  "packingList": {
    ...
    "proTip": "Pack a compact umbrella — afternoon showers likely"
          ...
          <Endpoint method="POST" path="/api/outfit-image" description="Generate a fashion editorial image for an outfit. Returns a base64-encoded PNG.">
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-[#a09080] font-medium mb-3">Request body</p>
              <CodeBlock>{`{
  "outfit": {
    "headline": "Olive layers, warm neutrals",
    "walkOut": {
  ...
  "image": "data:image/png;base64,iVBOR..."
              ...
              <p className="text-[12px] text-[#b0a490] mt-2">
        ...
        <div className="mt-14 space-y-6">
          <h2 className="font-[var(--font-serif)] text-[24px] text-[#3a3530]">Notes</h2>
          <div className="space-y-4 text-[14px] text-[#8a8078] leading-relaxed">
            <p>
              <span className="font-medium text-[#5a5248]">Weather data</span> comes from Open-Meteo and covers 7-day forecasts. For trip dates beyond the forecast window, historical averages are used instead.
            </p>
            <p>
              <span className="font-medium text-[#5a5248]">Outfit generation</span> uses Claude Sonnet to reason about temperature, UV, wind, and precipitation — then recommends specific garments with colors and fabrics.
            </p>
            <p>
              <span className="font-medium text-[#5a5248]">Rate limits</span> are 50 requests per hour per IP address. If you need higher limits for production use, reach out.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Footer */}
      <footer className="max-w-[860px] mx-auto px-6 py-10 border-t border-[#e8e0d4] flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <img src="/logo-1.svg" alt="WeatherWear" className="w-5 h-5 opacity-50" />
          <span className="text-[13px] text-[#b0a490]">WeatherWear</span>
        </Link>
        <Link href="/app" className="text-[13px] text-[#b0a490] hover:text-[#6b7c5e] transition-colors">Open App</Link>
      </footer>
    </main>
  );
}
